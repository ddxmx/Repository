 ...
 * |- @Retention(RetentionPolicy.RUNTIME)：注解不仅被保存到class文件中，jvm加载class文件之后，仍然存在，可以通过反射机制读取注解信息
 * |- @Retention(RetentionPolicy.SOURCE)：注解只保留在源文件，当Java文件编译成class文件的时候，注解被遗弃
 * |- @Retention(RetentionPolicy.CLASS)（默认）：注解被保留到class文件，但jvm加载class文件时候被遗弃
 * (2)、@Target：注解的作用目标
