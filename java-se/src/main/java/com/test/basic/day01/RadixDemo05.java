...
/**
 * 数的进制表示
 * 1、常见的进制表示形式有四种：二进制、八进制、十进制和十六进制
 * 二进制 0b开头，八进制 0开头，十六进制 0x开头
 * 2、计算机底层都是使用补码形式存储数据
 * 计算机底层进行减法运算时，实际上也是用的加法，1 - 1 = 1 + (-1)
 * （1）正数的原码、反码、补码都一致
 * （2）负数的原码 = 符号位为1，其余位和对应的正数一致
 * （3）负数的反码 = 负数的原码除符号位外，按位取反
 * （4）负数的补码 = 负数的反码 + 1
 * 3、为什么计算机底层都是使用补码方式进行计算？
 * （1）使用原码的方式 1-1=1+(-1)=0000_0001 + 1000_0001 = 1000_0010，不用原码表示是因为原码计算结果不正确
 * （2）使用补码的方式 1-1=1+(-1)=0000_0001 + 1111_1111 = 0000_0000 = 0
 */
public class RadixDemo05 {
    public static void main(String[] args) {
        ...
        /*
            ...
            二进制和八进制、十六进制转换：
            （1）二进制转八进制、十六进制：补码三位变一位、四位变一位
            （2）八进制、十六进制转二进制：补码一位变三位、一位变四位
         */
