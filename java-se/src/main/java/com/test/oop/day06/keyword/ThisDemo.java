...
/**
 * 1、this的不同使用方式
 * （1）this.属性  明确的表示当前的属性是类中的属性，用于在方法区分同名形参
 * （2）this.方法  明确的表示调用的是类中的方法
 * （3）this() 表示调用类中的其他构造器
 * （4）this表示当前对象（当前哪个对象在调用该方法，哪个对象就是当前对象）
 * 2、this调用类中其他构造器的语句，要放在构造器的首行，且不能形成递归调用
