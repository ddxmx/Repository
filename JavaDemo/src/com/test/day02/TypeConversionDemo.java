...
public class TypeConversionDemo {
    public static void main(String[] args) {
        //自动类型转换
        {
            ...
            byte b3 = 10;
            short s3 = 20;
            //short sum3 = b3 + s3;  //编译失败，byte、short、char进行运算时，很容易计算结果溢出，因此java先转换为int类型再计算
            int sum3 = b3 + s3;
            System.out.println(sum3); //30

            int i4 = 10;
            char c4 = 'a';
            //char类型会转换为int类型计算，char类型 转换为表示的Unicode编码值，a编码为97
            int sum4 = i4 + c4;
            System.out.println(sum4); //107

            byte sum5 = 10 + 20; //编译通过，常量计算结果在byte范围内，直接赋值
            //byte sum6 = 10 + 118; //编译失败，常量计算结果已经超过byte的范围
            byte b5 = 10;
            byte b6 = 20;
            //byte sum7 = b5 + b6; //编译失败，b5和b6都是变量，无法保证计算结果在byte范围内，需要使用int类型接收
            int sum7 = b5 + b6;
            System.out.println(sum7); //30

            final byte b7 = 10;
            final byte b8 = 20;
            byte sum8 = b7 + b8; //b7和b8都是常量，计算结果在byte范围内，等价于 byte sum8 = 10 + 20
        ...
        //强制类型转换
        {
            double d1 = 10.8;
            int i1 = (int) d1;
            System.out.println(i1); // 10，小数转换为整数，直接舍弃小数位
