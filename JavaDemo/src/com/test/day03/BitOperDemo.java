...
public class BitOperDemo {
    public static void main(String[] args) {
        ...
        */
        ...
        /*
            获取一个数字补码形式后N位的方式
            如：获取int类型数字20的补码形式后4位：0001 0100
         */
        ...
        int decimal = 141;
        //调用系统API方式获取
        System.out.println("调用系统API方式：" + Integer.toHexString(decimal)); //调用系统API方式：8d
        /*
            自己实现方式
            数字表示为十六进制，先转换为二进制，然后四位变一位
         */
        //取出二进制数字的低四位
        int lowerDecimal = decimal & 0b1111;
        String lowerHexString = lowerDecimal > 9 ? String.valueOf((char) (lowerDecimal - 10 + 'a')) : String.valueOf(lowerDecimal);
        System.out.println("lowerHexString:" + lowerHexString); //lowerHexString:d
        //右移四位后，高四位变成低四位
        int higherDecimal = decimal >>> 4 & 0b1111;
        String higherHexString = higherDecimal > 9 ? String.valueOf((char) (higherDecimal - 10 + 'a')) : String.valueOf(higherDecimal);
